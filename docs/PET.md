...
| B-036 | Relecture indépendante avant publication (2026-08-30) — 3 défauts en forme de pollution de prototype `__proto__`, trouvés par un sub-agent Code Review Master. (1)+(2) `profile-hints.ts`/`human-design-profile.ts` (B-031/B-033, écrits cette session) : une clé calculée `{ ...x, ['__proto__']: y }` crée une vraie propriété propre que `Zod.strict()` ignore silencieusement au lieu de la rejeter — corrigé par un allowlist explicite (`assertKnownKeys`, lit `Object.keys`) exécuté avant Zod dans les deux modules. (3) `sync-engine.ts` (B-016, préexistant) : mécanisme différent — `getSyncedPreferences()` reconstruit un objet `{}` par affectation `result[key] = value`, et sur un objet simple existant `obj['__proto__'] = y` invoque le setter hérité de `Object.prototype` au lieu de créer une propriété propre (donc jamais visible même dans `Object.keys`) ; corrigé par `Object.create(null)` comme accumulateur — un objet sans prototype n'a pas ce setter. 3 tests déterministes ajoutés (un par fichier) démontrant le cas exact trouvé, en plus de la couverture PBT existante. Chacune des deux mécaniques a nécessité un correctif distinct — même famille de défaut, deux causes différentes. | — | 🟢 Done | **Sensitive 90%+ / Critical 95%** maintenus (sync-engine.ts 96.72% stmts/lines, 92.85% branches ; profile-hints.ts + human-design-profile.ts 100%) | — (aucune nouvelle dépendance) | _à compléter au push_ | 2026-08-30 |
| B-037 | `<MorphicSuggestions>` — premier branchement du profil holistique dans l'adaptateur publiable. Reçoit un `ProfileHints` déjà classé fourni par l'hôte (jamais calculé ici — CDC F-037 : l'instrument HSP/ASRS/GAD reste côté hôte) et affiche les suggestions de `suggestAxesFromProfileHints` sous forme de liste : chaque ligne porte sa raison en clair + deux actions explicites, "Appliquer" (route vers le vrai setter/action moteur — `setMotion`/`setDensity`/`startPomodoro`/`enterRecoveryMode`) et "Ignorer" (retire la ligne sans toucher au moteur). Rien n'est jamais appliqué sans clic (Dignity §c, libre arbitre préservé). Liste vide (aucun trait `high`) → composant ne rend rien. 12 tests (état vide, rendu multi-suggestions, application par axe vérifiée via les vrais getters moteur — pas de mock —, ignorer, labels FR surchargeables). | F-037 | 🟢 Done | **Standard 80%** ✅ (atteint 100% stmts/lines, 87.5% branches, 100% funcs) | react@19.2.5 (déjà vérifié B-030j) | _à compléter au push_ | 2026-08-30 |
...
| B-035 | Axe **Pomodoro Strip** — refonte complète sur retour de Jay 2026-08-30 après avoir vu B-034 en vrai dans la maquette (*"la barre doit fonctionner différemment"*). Bandeau à deux couches : piste gris pâle fixe + remplissage qui grandit et change de couleur en continu. Quand une phase active se termine vers une autre phase active (`work`↔pause), le bandeau clignote lentement en vert pendant 4s (`prefers-reduced-motion` : vert fixe, sans animation) puis reprend le remplissage de la nouvelle phase. Un arrêt manuel (`stopPomodoro`) ne déclenche jamais la respiration — seul un passage naturel entre deux phases actives compte comme "le cycle se termine", pas une annulation. Sonde `getPomodoroState()` toutes les 1s (même raison que B-034 : `skipPhase`/`stopPomodoro` n'émettent pas toujours un event ; latence visuelle max 1s, vérifiée en relisant le code — pas de vrai retard). Durée totale de la phase capturée au premier `remainingMs` observé après un changement de phase. Branché dans la maquette (`demo/main.tsx`) pour vérification visuelle par Jay avant publication. | F-035 ext. | 🟢 Done | **Standard 80%** | — (aucune nouvelle dépendance, réutilise `pomodoro.ts`) | _à compléter au push_ | 2026-08-30 |
| B-035c | Retour Jay 2026-08-30 (2e passage) : *"il y a toujours un petit temps de décalage avant que le liseré n'apparaisse"*. Sondage passé de 1s à 250ms — le délai venait de la cadence de sondage elle-même (jusqu'à ~1,2s entre le clic "Démarrer" et le premier rafraîchissement visuel, sondage + transition d'opacité 200ms cumulés), pas d'un vrai bug. Ajout dans la maquette (`demo/main.tsx`, outil de test seulement) de 3 sélecteurs de couleur en direct — `enablePomodoroStrip()` accepte déjà `startColor`/`midColor`/`endColor` en option depuis B-035, ceci évite un aller-retour commit à chaque nuance testée. 27 tests inchangés (constante référencée par symbole partout, aucune valeur en dur cassée). | F-035 ext. | 🟢 Done | **Standard 80%** ✅ | — (constante interne, pas de dépendance) | _à compléter au push_ | 2026-08-30 |
| B-035b | Retour Jay 2026-08-30 après test réel dans la maquette : *"je ne vois aucune progression"* + *"il faudrait que le bleu soit un peu plus visible"*. Diagnostic : le gradient à 2 segments de B-035 (bleu atteint seulement à 75% de la phase) restait quasi indiscernable du gris pendant la majeure partie du temps — pas un bug de latence (confirmé par relecture du code), un problème de contraste. Passage à un gradient à **3 segments** : bleu vif atteint dès 35% de la phase (`POMODORO_STRIP_DEFAULT_RAMP_UP_STOP`), plateau bleu jusqu'à 85% (`POMODORO_STRIP_DEFAULT_RAMP_DOWN_STOP`), orange seulement dans les 15% finaux. Couleur bleue elle-même rendue plus saturée (`#3b82f6` au lieu de `#60a5fa`). `computePomodoroStripFillColor` passe de 4 à 5 paramètres (ajout `rampDownStop`), tous les tests de couleur réécrits avec de nouvelles valeurs de référence calculées à la main. 27 tests (2 de plus qu'avant : plateau + valeur exacte au second point de rupture). | F-035 ext. | 🟢 Done | **Standard 80%** ✅ (atteint 97.63% stmts / 98.33% lines / 100% funcs / 93.33% branches) | — (aucune nouvelle dépendance) | _à compléter au push_ | 2026-08-30 |
