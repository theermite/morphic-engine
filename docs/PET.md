...
### Phase 1.7 — Bouton morphique publiable (B-030a à B-030h)
...
| ID | Brick | CDC ref | Statut | Coverage cible | Veille requise | Commit | Date |
|----|-------|---------|--------|----------------|----------------|--------|------|
| B-034 | Axe **Pomodoro Strip** — bandeau fin (`position: fixed`, 4px par défaut) qui change de couleur selon la phase pomodoro (`work`/`short-break`/`long-break`/`idle` masqué). Même langage visuel que le liseré desktop de Hibiki (fenêtre OS dédiée) — ici en overlay DOM puisque le moteur n'a que la page web. `enablePomodoroStrip(options?)` / `disablePomodoroStrip()` / `getPomodoroStripState()`. Sonde `getPomodoroState()` toutes les 1s (cadence alignée sur le tick pomodoro) plutôt que de dépendre des events de `pomodoro.ts` — `skipPhase`/`stopPomodoro` n'émettent pas d'event à chaque transition, la sonde reste donc correcte même sans eux (décalage visuel max 1s). Aucune nouvelle préférence persistée — visuel compagnon d'une session déjà gérée par `pomodoro.ts`. `prefers-reduced-motion` coupe la transition, jamais le bandeau (porte une information d'état, pas une décoration). 13 tests dont 3 sur l'auto-correction par sonde (skip/stop sans event, arrêt propre de la sonde à la désactivation). | F-035 ext. | 🟢 Done | **Standard 80%** (atteint 91.37% stmts / 94.23% lines / 100% funcs / 88.63% branches) | — (aucune nouvelle dépendance, réutilise `pomodoro.ts`) | _à compléter au push_ | 2026-08-29 |
